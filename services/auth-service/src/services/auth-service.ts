...
import { BadRequestError, ConflictError, HttpError, InternalServerError, UnauthorizedError } from "../errors/http-errors";
...
export class AuthService {
  ...
  public async login(payload: LoginPayload): Promise<AuthTokens> {
    ...
    try {
      ...
      const passwordMatch = await this.passwordService.verify(user.password, payload.password);
      if (!passwordMatch) throw new UnauthorizedError(`Your credentials are incorrect!`);
